...
- **Who reviews when you omit `reviewer`:** the reviewer is resolved, never defaulted to you. Order: the reviewer recorded on the latest `changes_requested` event (re-review), then `kanban.default_reviewer`. If that config is unset, names a profile that is not installed, or names the implementer's own profile, the call **fails closed** with an actionable reason — the card stays `running`, the live claim is untouched, and nothing enters `review`. Set `kanban.default_reviewer` to a distinct reviewer profile, or pass `reviewer=` explicitly.
